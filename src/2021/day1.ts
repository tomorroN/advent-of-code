...
const calculatePart1 = (input: string[]): number => {
  let counter = 0;
  input.map(Number).reduce((previousValue, currentValue) => {
...
const calculatePart2 = (input: string[]): number => {
  ...
  input
    ...
    .reduce((previousSlicingWindowSum, currentValue, currentIndex, array) => {
      const currentSlicingWindowSum =
        array[currentIndex] + array[currentIndex + 1] + array[currentIndex + 2];
      if (
        previousSlicingWindowSum &&
        currentSlicingWindowSum > previousSlicingWindowSum
      ) {
        counter += 1;
      }
